...
const global data_path = "/gpfs/work/pn68va/di67meg/PaperRepos/SynchWeb/maps/coma/zoom_inj_new/"
const global sim_path = "/gpfs/work/pn68va/di67meg/LocalUniverseZoom/Coma/L5/mhd_cr6p24e/"
...
include("/gpfs/work/pn68va/di67meg/PaperRepos/SynchWeb/scripts/allsky/Bfld.jl")
include("/gpfs/work/pn68va/di67meg/PaperRepos/SynchWeb/scripts/allsky/synchrotron.jl")
...
function make_synch_maps(snap, cluster, gpos, side_length, scale, map_type)
    ...
    if map_type == "flux"
        ...
        units = "mJy"
        synch = "synchF"
        factor = m_cgs ./ rho_cgs
        weights = part_weight_physical(length(rho_cgs), param)
        flux = true
        reduce_image = false

    elseif map_type == "luminosity"

        units = "W/Hz"
        synch = "synch_Pnu"
        factor = m_cgs ./ rho_cgs
        weights = ones(length(m_cgs))
        flux = false
        reduce_image = true
...
function make_quantity_maps(snap, cluster, gpos, side_length, scale)
    ...
    # #         units="g/cm^2", reduce_image=true; 
    # #         kernel, snap, param, image_prefix)
    # map_it(pos, hsml, mass, rho, rho_cgs, weights, 
    ...
    # image_prefix = image_path * "CReP"
    # CReP = data["CReP"] .* GU.P_CR_cgs
    # # map_it(pos, hsml, mass, rho, CReP, rho, 
    # #         units="erg/cm^3", calc_mean=false; 
    # #         kernel, snap, param, image_prefix)
    # map_it(pos, hsml, mass, rho, CReP, rho, 
    #         units="erg/cm^3", calc_mean=false,
    #         projection="xz"; 
...
function make_analytic_synch_maps(snap, cluster, gpos, side_length, scale, map_type)
    ...
    elseif map_type == "SB"
        ...
        units = "erg/s/Hz/cm^2"
        synch = "synch_Inu_analytic_144MHz"
        factor = ones(length(m_cgs))
        weights = part_weight_physical(length(rho_cgs), param)
        flux = false
        reduce_image = false
    ...
    j_ν = analytic_synchrotron(Pth, B, mach, ν0=nu, CR_Emin=5.11e-5)
...
# # hercules
# gpos = [219.701, 316.870, 338.448] .* 1.e3
# cluster = "hercules"
...
# # power
# make_synch_maps(74, cluster, gpos, side_length, "20Mpc", "luminosity")

# # flux
# make_synch_maps(74, cluster, gpos, side_length, "20Mpc", "flux")
