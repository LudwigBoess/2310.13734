...
function write_baseline_image(map_base, snap, Bname, smooth_size)
    ...
    im_baseline[ im_baseline .< -1.0 ] .= -1.0
    im_baseline[ isnan.(im_baseline) ] .= -1.0
    im_baseline[ isinf.(im_baseline) ] .= -1.0
    println("saving baseline image: $filename_out")
...
# files = @. map_path * "/zoom_inj/coma_20Mpc_$(@sprintf("%03i", 74)).baseline_" * Bfield_names * ".xz.fits"
...
# """
# Bfield_names = ["Bsim", "beta50", "01Pturb", "BFF", "dyn_l", "dyn_h"]
# files = @. map_path * "zoom_inj/coma_20Mpc_$(@sprintf("%03i", 74)).synch_F_beam_1'_144MHz_boosted_" * Bfield_names * ".xz.fits"
