...
# Load data
#load_file(filename) = readdlm(filename)

function plot_spectra(data_path, folders, sim_names, Bfield_names, plot_name)
    ...
    for i = 0:Ncols-1
        ...
        filename = data_path * "$(folders[i+1])/f_spectrum.dat"
        ...
        for j = 1:length(Bfield_models)
            filename = data_path * "$(folders[i+1])/synch_spectrum_$(Bfield_names[j]).dat"
            data = readdlm(filename)
...
data_path = "/gpfs/work/pn68va/di67meg/PaperRepos/SynchWeb/data/spectra/"
...
plot_name = "/gpfs/work/pn68va/di67meg/PaperRepos/SynchWeb/Plots/cr_spectra.pdf"
plot_spectra(data_path, folders, sim_names, Bfield_names, plot_name)